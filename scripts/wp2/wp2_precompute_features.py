...
from .wp2_functions import drf, drf_wl, its_wl_feature_sets
    ...
    *,
    ...
    if rxn_col is None:
        if "clean_rxn" not in df.columns:
            raise KeyError("Required column 'clean_rxn' not found in DataFrame. "
    ...
    for i in it:
        ...
        try:
            ...
            total = drf_wl(
                ...
                digest_size=digest_size,
                return_per_iter=False,
            )
            drf_list.append(base)
            wl_drf_list.append(total)
        except Exception as e:
            errors.append({"index": i, "rsmi": rsmi, "error": repr(e)})
            if stop_on_error:
                raise
